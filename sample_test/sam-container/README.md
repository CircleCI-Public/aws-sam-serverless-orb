...
```bash
sam-container$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-container$ sam local start-api
sam-container$ curl http://localhost:3000/
```
...
```bash
sam-container$ sam logs -n HelloWorldFunction --stack-name sam-container --tail
```
...
```bash
aws cloudformation delete-stack --stack-name sam-container
```
